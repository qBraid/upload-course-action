...
class TestImageValidator:
    ...
    def test_check_notebook_images_valid(
        self, mock_stat, mock_exists, mock_open, mock_nb_read
    ):
    ...
    @mock.patch("os.path.exists")
    @mock.patch("check_images.Path.stat")
    def test_check_notebook_images_oversized(
        self, mock_stat, mock_exists, mock_open, mock_nb_read
    ):
    ...
    def test_verify_images_success(self, mock_check, mock_exists, mock_json, mock_open):
        ...
        mock_json.return_value = {
            "content": [
                {
                    "baseFilePath": "nb1.ipynb",
                    "sections": [{"baseFilePath": "nb2.ipynb"}],
                }
            ]
        }
    ...
    def test_verify_images_failed(self, mock_exists, mock_json, mock_open):
        ...
        with mock.patch.object(
            self.validator,
            "check_notebook_images",
            side_effect=lambda x: self.validator.image_issues.append(
                {
                    "issue": "missing",
                    "notebook": x,
                    "type": "absolute",
